...
function App() {
  ...
  useEffect(() => {
    ...
    // Listen for Cmd+K, Spacebar and Shift
    ...
    <div className="w-screen h-screen relative overflow-hidden bg-background mac-drag-region flex flex-col">
      ...
      <div className="absolute top-0 w-[50%] left-1/2 -translate-x-1/2 mt-4 h-12 z-40 bg-[#131315]/80 backdrop-blur-2xl flex items-center px-6 pointer-events-auto rounded-full border border-outline-variant/20 shadow-[0px_12px_32px_rgba(255,255,255,0.1)] no-drag-region justify-between">
        ...
        <div className="flex items-center gap-4 w-full max-w-sm justify-center">
          <div onClick={() => setOmnibarOpen(true)} className="flex-1 flex items-center gap-2 bg-[#2a2a2c] rounded-full px-3 py-1.5 border border-[#414754]/50 hover:border-[#aac7ff]/50 transition-colors cursor-pointer group">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#8b91a0] group-hover:text-[#aac7ff] transition-colors"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.3-4.3"></path></svg>
            <span className="text-xs text-[#8b91a0] group-hover:text-white transition-colors">Search or launch tab...</span>
            <kbd className="px-1.5 py-0.5 bg-[#353437] rounded text-[10px] text-[#c0c6d6] ml-auto">⌘K</kbd>
          </div>
        </div>
